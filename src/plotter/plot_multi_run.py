...
for keys, values in heuristic_raw_results.items():
    ...
    # list_of_1_hour_tardy = [dept for dept in flat_list if dept >= 15]
    # pct_tardy = len(list_of_1_hour_tardy) / len(flat_list)
...
fig = go.Figure(data=[
    ...
    go.Surface(z=bau_result, x=L2_STATIONS, y=VEHICLES, opacity=0.2),
    go.Surface(z=heuristic_result, x=L2_STATIONS, y=VEHICLES, opacity=0.2)],
